...
static int are_symbols_valid(char **map)
{
	...
	while (map[i])
	{
		...
		while (map[i][j])
		{
			...
			{
				fprintf(stderr, "Error: Unknown symbol '%c' (-> \"%s\")\n",
						map[i][j], "01BCEGP");
				return 0;
			}
			++j;
...
static int are_symbols_in_right_amount(char **map)
{
	...
	if (amount[0] >= 1 && amount[1] == 1 && amount[2] == 1)
		return 1;
	if (!amount[0])
		fprintf(stderr, "Error: No collectible found ('C')\n");
	if (!amount[1])
		fprintf(stderr, "Error: No exit found ('E')\n");
	else if (amount[1] > 1)
		fprintf(stderr, "Error: More than one exit found ('E')\n");
	if (!amount[2])
		fprintf(stderr, "Error: No player start found ('P')\n");
	else if (amount[2] > 1)
		fprintf(stderr, "Error: More than one player start found ('P')\n");
	return 0;
}
...
static int is_map_rectangular_and_closed_off(char **map)
{
	...
	{
		fprintf(stderr, "Error: Map is not closed_off_by_walls\n");
		return 0;
	}
	return 1;
}
